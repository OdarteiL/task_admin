...
export default function TeamDashboard() {
  ...
  const fetchTasks = async () => {
    ...
    try {
      const res = await fetch(`https://okswggf9u7.execute-api.us-east-1.amazonaws.com/tasks?assignedTo=${userEmail}`)
  ...
  const updateTask = async (taskId: string, updates: Partial<Task>) => {
    try {
      ...
      const res = await fetch(`https://okswggf9u7.execute-api.us-east-1.amazonaws.com/tasks/${taskId}`, {
        method: "PUT",
      ...
      setTasks((prevTasks) =>
        prevTasks.map((task) => (task.id === taskId ? { ...task, ...updatesWithTimestamp } : task)),
      )
      ...
      <div className="max-w-5xl mx-auto p-6">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-xl shadow-lg p-6 text-white mb-8">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-blue-100 mt-1">Welcome, {userEmail}</p>
            </div>
            <button
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <textarea
                              ...
                              rows={3}
                              value={
                                (editedTasks[task.id]?.description !== undefined
                                  ? editedTasks[task.id].description
                                  : task.description) || ""
                              }
                          ...
                          <div className="flex flex-wrap gap-3 items-end">
                            <div className="flex-grow">
                              ...
                              <select
                                ...
                                value={
                                  editedTasks[task.id]?.status !== undefined ? editedTasks[task.id].status : task.status
                                }
            ...
            {message && (
              <div
                className={`mt-6 p-3 rounded-md ${message.includes("❌") ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"}`}
              >
                {message}
              </div>
            )}
